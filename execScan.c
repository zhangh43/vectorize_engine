			  ...
			  ExecScanRecheckMtd recheckMtd)
{
	...
	/* fetch the real ScanState */
	ScanState		*scanNode = (ScanState *)vss->seqstate;
	EState	   *estate = scanNode->ps.state;
	...
	if (estate->es_epq_active != NULL)
	{
		 ...
		 * conditions.
		 */
		Index		scanrelid = ((Scan *) scanNode->ps.plan)->scanrelid;
	...
	=		 * This is a ForeignScan or CustomScan which has pushed down a
			...
			TupleTableSlot *slot = scanNode->ss_ScanTupleSlot;
		 ...
		 ExecScanRecheckMtd recheckMtd)
{
	...
	if (!qual && !projInfo)
	{
		...
		return ExecScanFetch(ss, accessMtd, recheckMtd);
	}
	...
	for (;;)
	{
		...
		slot = ExecScanFetch(ss, accessMtd, recheckMtd);
		 ...
		 * ...
		 *
		 * Vectorize: ExecQual needs to consider the inputs are vector instead of scalar.
		 */
		if (qual == NULL || VExecQual(qual, econtext))
		{
			/*
...
VExecAssignScanProjectionInfo(ScanState *node)
{
    ...
    vdesc = CreateTupleDescCopyConstr(node->ss_ScanTupleSlot->tts_tupleDescriptor);
	...
	VectorExecConditionalAssignProjectionInfo(&node->ps, vdesc, scan->scanrelid);
	
	//TODO
	if (!node->ps.ps_ResultTupleSlot)
	{
		ExecInitResultSlot(&node->ps, &TTSOpsVirtual);
		node->ps.resultops = &TTSOpsVirtual;
		node->ps.resultopsfixed = true;
		node->ps.resultopsset = true;
	}
}
 ...
 */
void
VExecAssignScanProjectionInfoWithVarno(ScanState *node, Index varno)
{
	TupleDesc	vdesc;
	int 		i;
	...
	ExecConditionalAssignProjectionInfo(&node->ps, vdesc, varno);
}
 ...
 */
void
VExecScanReScan(ScanState *node)
{
