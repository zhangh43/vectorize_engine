...
bool
vaocs_getnext(AOCSScanDesc scan, ScanDirection direction, TupleTableSlot *slot)
{
...
ReadNext:
		...
		if (scan->cur_seg < 0 || err < 0)
		{
			...
			for (i = 0; i < scan->num_proj_atts; i++)
			{
				...
				scan->ds[attno]->blockRead.nth = 0;
				scan->ds[attno]->blockRead.physical_datum_index = 0;
			}
		}
		...
		for (i = 0; i < scan->num_proj_atts; i++)
		{
			...
			dsr = &scan->ds[attno]->blockRead;
			if (minimalBatchSize > dsr->logical_row_count - dsr->nth)
				minimalBatchSize = dsr->logical_row_count - dsr->nth;
		}
		...
		{
			int			attno = scan->proj_atts[i];
			vtype		*column = (vtype *)DatumGetPointer(d[attno]);
		...
		for (i = 0; i < scan->num_proj_atts; i++)
		{
			 ...
			 */
			vdatumstreamread_get(scan->ds[attno], &d[attno], &null[attno], minimalBatchSize);
			...
			{
				elog(ERROR, "not support old version data");
		...
		/* scan ctid is not supported yet */
		if (rowNum == INT64CONST(-1))
		{
