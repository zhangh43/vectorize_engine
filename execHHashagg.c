 ...
 */
static inline void
setGroupAggs(HashAggTable *hashtable, HashAggEntry *entry, int i)
{
	if (entry != NULL)
	{
 ...
 */
HashAggTable *
vcreate_agg_hash_table(AggState *aggstate)
{
	int keywidth;
	double entrywidth;
...
vagg_hash_initial_pass(AggState *aggstate)
{
	...
	while(true)
	{
		...
		for (i = 0; i < BATCHSIZE; i++)
		{
			...
			setGroupAggs(hashtable, entry, i);
...
agg_hash_stream(AggState *aggstate)
{
	...
	return vagg_hash_initial_pass(aggstate);
}
...
{
	elog(ERROR, "agg_hash_reload not supported yet.");
}
/*
