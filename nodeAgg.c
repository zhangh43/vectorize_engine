...
static TupleTableSlot *agg_retrieve_hash_table(VectorAggState *vas);
...
static void Vadvance_aggregates(AggState *aggstate, TupleHashEntryData **entries);
...
InitAggResultSlot(VectorAggState *vas, EState *estate)
{
	...
	vas->resultSlot = VExecInitExtraTupleSlot(estate, vdesc);
	...
	for (i = 0; i < vdesc->natts; i++)
	{
		...
		typid = TupleDescAttr(vas->resultSlot->tts_tupleDescriptor, i)->atttypid;
 ...
 */
static void
Vadvance_aggregates(AggState *aggstate, TupleHashEntryData **entries)
{
	int			transno;
	int			setno = 0;
	...
	for (transno = 0; transno < numTrans; transno++)
	{
		...
		ExprState  *filter = pertrans->equalfnMulti;
		...
		econtext->ecxt_outertuple = aggstate->tmpcontext->ecxt_outertuple;

		/* slot = ExecProject(pertrans->evalproj, NULL); */
		slot = ExecProject((ProjectionInfo*)pertrans->sortslot);
		...
		else
		{
			...
			for (i = 0; i < numTransInputs; i++)
			{
				PG_GETARG_DATUM(i + 1) = slot->tts_values[i];
				PG_ARGISNULL(i + 1) = slot->tts_isnull[i];
			}

			for (setno = 0; setno < numGroupingSets; setno++)
			{
				//AggStatePerGroup pergroupstate = &pergroup[transno + (setno * numTrans)];
				int groupOffset = transno + (setno * numTrans);
				aggstate->current_set = setno;

				fcinfo->fncollation = groupOffset;
							...
							TupleHashEntryData **entries)
{
	 ...
	 */
	PG_GETARG_DATUM(0) = PointerGetDatum(entries);
...
lookup_hash_entry(AggState *aggstate, TupleTableSlot *inputslot)
{
	...
	entries = palloc0(sizeof(TupleHashEntryData*) * BATCHSIZE);
...
agg_retrieve_direct(VectorAggState *vas)
{
	...
	while (!aggstate->agg_done)
	{
		...
		else
		{
			...
			if (aggstate->grp_firstTuple != NULL)
			{
				...
				for (;;)
				{
					...
					{
						elog(ERROR, "Mixed aggregates can not be vectorized now");
					}
		...
		for(i = 0; i < vdesc->natts; i++)
		{
			column = (vtype *)DatumGetPointer(vslot->tts.base.tts_values[i]);
...
agg_fill_hash_table(AggState *aggstate)
{
	...
	for (;;)
	{
		...
		/* TODO */
			...
			Vadvance_aggregates(aggstate, entries);
...
VExecInitAgg(Agg *node, EState *estate, int eflags)
{
	...
	aggstate->ss.ps.ExecProcNode = (ExecProcNodeMtd)VExecAgg;
...
//	ExecCreateScanSlotFromOuterPlan(estate, &aggstate->ss, aggstate->ss.ps.outerops);
	{
		...
		outerPlan = outerPlanState(&aggstate->ss);
		tupDesc = ExecGetResultType(outerPlan);
		VExecInitScanTupleSlot(estate, &aggstate->ss, tupDesc);
	...
	if (numPhases > 2)
	{
		aggstate->sort_slot = VExecInitExtraTupleSlot(estate, scanDesc);
	 ...
	 */
	ExecInitResultTypeTL(&aggstate->ss.ps);
						  ...
						  Oid *inputTypes, int numArguments)
{
	...
	{
		TupleDesc desc = ExecTypeFromTL(aggref->args);
		TupleTableSlot* evalslot = VExecInitExtraTupleSlot(estate, desc);
		ProjectionInfo* info = ExecBuildProjectionInfo(aggref->args,
													   aggstate->tmpcontext,
													   evalslot,
													   NULL,
													   NULL);
		pertrans->sortslot = (TupleTableSlot*)info; /* Dirty hack to reuse existed AggStatePerTransData */
	}
	...
	if (numSortCols > 0 || aggref->aggfilter)
	{
		...
		pertrans->sortslot = VExecInitExtraTupleSlot(estate, pertrans->sortdesc);
	}
	...
	if (numSortCols > 0)
	{
		...
		else if (numDistinctCols > 0)
		{
			...
			pertrans->uniqslot = VExecInitExtraTupleSlot(estate, pertrans->sortdesc);
		}
