 ...
 */
void
VectorExecReadyInterpretedExpr(ExprState *state)
{
	 ...
	 */
	state->evalfunc = VectorExecInterpExprStillValid;
...
ExecInterpExpr(ExprState *state, ExprContext *econtext, bool *isnull)
{
	...
	VectorTupleTableSlot *vresultslot;
	VectorTupleTableSlot *vinnerslot;
	VectorTupleTableSlot *vouterslot;
	VectorTupleTableSlot *vscanslot;
	/*
	...
	vresultslot = (VectorTupleTableSlot *)resultslot;
	vinnerslot = (VectorTupleTableSlot *)innerslot;
	vouterslot = (VectorTupleTableSlot *)outerslot;
	vscanslot = (VectorTupleTableSlot *)scanslot;

	//TODO hack here
	if (vresultslot && vscanslot)
	{
		vresultslot->dim = vscanslot->dim;
		memcpy(vresultslot->skip, vscanslot->skip, sizeof(bool) * BATCHSIZE);
	}
	...
	EEO_SWITCH()
	{
		...
		EEO_CASE(EEOP_INNER_FETCHSOME)
		{
			...
			vector_slot_getsomeattrs(innerslot, op->d.fetch.last_var);
		...
		EEO_CASE(EEOP_OUTER_FETCHSOME)
		{
			...
			vector_slot_getsomeattrs(outerslot, op->d.fetch.last_var);
		...
		EEO_CASE(EEOP_SCAN_FETCHSOME)
		{
			...
			vector_slot_getsomeattrs(scanslot, op->d.fetch.last_var);
		...
		EEO_CASE(EEOP_INNER_VAR)
		{
			...
			*op->resvalue = vinnerslot->vector_tts_values[attnum];
		...
		EEO_CASE(EEOP_OUTER_VAR)
		{
			...
			*op->resvalue = vouterslot->vector_tts_values[attnum];
		...
		EEO_CASE(EEOP_SCAN_VAR)
		{
			...
			*op->resvalue = vscanslot->vector_tts_values[attnum];
		...
		EEO_CASE(EEOP_ASSIGN_INNER_VAR)
		{
			 ...
			 */
			Assert(attnum >= 0 && attnum < innerslot->tts_nvalid);
			vresultslot->vector_tts_values[resultnum] = vinnerslot->vector_tts_values[attnum];
			resultslot->tts_isnull[resultnum] = innerslot->tts_isnull[attnum];
			memcpy(vresultslot->skip, vinnerslot->skip, sizeof(bool) * BATCHSIZE);
		...
		EEO_CASE(EEOP_ASSIGN_OUTER_VAR)
		{
			 ...
			 */
			Assert(attnum >= 0 && attnum < outerslot->tts_nvalid);
			vresultslot->vector_tts_values[resultnum] = vouterslot->vector_tts_values[attnum];
			resultslot->tts_isnull[resultnum] = outerslot->tts_isnull[attnum];
			memcpy(vresultslot->skip, vouterslot->skip, sizeof(bool) * BATCHSIZE);
		...
		EEO_CASE(EEOP_ASSIGN_SCAN_VAR)
		{
			 ...
			 */
			Assert(attnum >= 0 && attnum < scanslot->tts_nvalid);
			vresultslot->vector_tts_values[resultnum] = vscanslot->vector_tts_values[attnum];
		...
		EEO_CASE(EEOP_ASSIGN_TMP)
		{
			...
			vresultslot->vector_tts_values[resultnum] = state->resvalue;
		...
		EEO_CASE(EEOP_ASSIGN_TMP_MAKE_RO)
		{
			...
			if (!resultslot->tts_isnull[resultnum])
				vresultslot->vector_tts_values[resultnum] =
	...
	strictfail:
		...
		EEO_CASE(EEOP_QUAL)
		{
			...
			/* vectorize qual needs to set skip array */
 ...
 */
Datum
VectorExecInterpExprStillValid(ExprState *state, ExprContext *econtext, bool *isNull)
{
	/*
	 * First time through, check whether attribute matches Var.  Might not be
	 * ok anymore, due to schema changes.
	 */
	VectorCheckExprStillValid(state, econtext);
 ...
 */
void
VectorCheckExprStillValid(ExprState *state, ExprContext *econtext)
{
	TupleTableSlot *innerslot;
	TupleTableSlot *outerslot;
	TupleTableSlot *scanslot;
	...
	for (int i = 0; i < state->steps_len; i++)
	{
		...
		switch (VectorExecEvalStepOp(state, op))
		{
			case EEOP_INNER_VAR:
				{
					int			attnum = op->d.var.attnum;
...
CheckVarSlotCompatibility(TupleTableSlot *slot, int attnum, Oid vartype)
{
	...
	if (attnum > 0)
	{
		TupleDesc	slot_tupdesc = vslot->vector_tts_tupleDescriptor;
...
ExecJustVarImpl(ExprState *state, TupleTableSlot *slot, bool *isnull)
{
	 ...
	 */
	return vector_slot_getattr(slot, attnum, isnull);
}
...
ExecJustAssignVarImpl(ExprState *state, TupleTableSlot *inslot, bool *isnull)
{
	...
	VectorTupleTableSlot *voutslot = (VectorTupleTableSlot *)outslot;
	VectorTupleTableSlot *vinslot = (VectorTupleTableSlot *)inslot;
	 ...
	 * --- slot_getattr() will take care of any problems.
	 */
	voutslot->vector_tts_values[resultnum] =
		vector_slot_getattr(inslot, attnum, &outslot->tts_isnull[resultnum]);
	voutslot->dim = vinslot->dim;
	memcpy(voutslot->skip, vinslot->skip, sizeof(bool) * BATCHSIZE);
	return 0;
}
...
ExecJustVarVirtImpl(ExprState *state, TupleTableSlot *slot, bool *isnull)
{
	 ...
	 */
	Assert(TTS_IS_VECTOR_VIRTUAL(slot));
	Assert(TTS_FIXED(slot));
	Assert(attnum >= 0 && attnum < slot->tts_nvalid);

	*isnull = false;

	return vslot->vector_tts_values[attnum];
}
...
ExecJustAssignVarVirtImpl(ExprState *state, TupleTableSlot *inslot, bool *isnull)
{
	...
	voutslot->vector_tts_values[resultnum] = vinslot->vector_tts_values[attnum];
 ...
 */
ExprEvalOp
VectorExecEvalStepOp(ExprState *state, ExprEvalStep *op)
{
#if defined(EEO_USE_COMPUTED_GOTO)
...
ExecEvalWholeRowVar(ExprState *state, ExprEvalStep *op, ExprContext *econtext)
{
	 ...
	 */
	/*TODO: vectorize toast not supportted yet*/
